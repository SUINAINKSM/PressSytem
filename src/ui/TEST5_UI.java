...
public class TEST5_UI extends JFrame {
	...
	public TEST5_UI() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if( selection == JOptionPane.OK_OPTION ){
					try {
						new EBiz().deleteBookName(bookName);
					...
					updateTable();
					try {
						label_1.setText("共"+new EBiz().getBjsnameListNumber()+"项");
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
		...
		button_1.addActionListener(new ActionListener() {
			...
			if(label.getText().equals("添加")){
			   ...
			   try {
				...
				int flag=new EBiz().AddBjsInformation(bjsname, bookType, shum);
				if(flag==0){//注册失败
				...
				try {
					...
					String bookType=bookTypeComboBox.getSelectedItem().toString();
					String shum=bookNameTextField.getText().toString();
				    if(new EBiz().getBookName(uid).equals(bookNameTextField.getText().toString())==false){
				    	int flag=new EBiz().updateBookName(uid, shum);
						if(flag==0){
							JOptionPane.showMessageDialog(null,"已有同名图书，该图书名称修改失败!","错误",JOptionPane.ERROR_MESSAGE);
							bookNameTextField.requestFocus();
						}
				    }
				    if(new EBiz().getBjsName(uid)!=bjsname){
				    	int flag=new EBiz().updateBjsName(uid, bjsname);
				    }
				    if(new EBiz().getBookType(uid)!=bookType){
				    	int flag=new EBiz().updateBookType(uid, bookType);
				    }
					updateTable();
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
        ...
        try {
			bjsNameList=new EBiz().getBjsNameList();
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			bookTypeList=new EBiz().getBookTypeNameList();
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		bookNameTextField.setFont(new Font("宋体", Font.PLAIN, 18));
		bookNameTextField.setBounds(140, 489, 196, 25);
