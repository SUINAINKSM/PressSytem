...
public class TEST6_UI extends JFrame {
	...
	public TEST6_UI() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if( selection == JOptionPane.OK_OPTION ){
					try {
						new FBiz().deleteBookName(bookName);
					...
					updateTable();
					try {
						label_1.setText("共"+new FBiz().getBjsnameListNumber()+"项");
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
		...
		button_1.addActionListener(new ActionListener() {
			...
			if(label.getText().equals("添加")){
			   ...
			   try {
				...
				int flag=new FBiz().AddBjsInformation(bjsname, bookType, shum);
				if(flag==0){//注册失败
				...
				try {
					...
					String bookType=bookTypeComboBox.getSelectedItem().toString();
					String shum=bookNameTextField.getText().toString();
				    if(new FBiz().getBookName(uid).equals(bookNameTextField.getText().toString())==false){
				    	int flag=new FBiz().updateBookName(uid, shum);
						if(flag==0){
							JOptionPane.showMessageDialog(null,"已有同名图书，该图书名称修改失败!","错误",JOptionPane.ERROR_MESSAGE);
							bookNameTextField.requestFocus();
						}
				    }
				    if(new FBiz().getBjsName(uid)!=bjsname){
				    	int flag=new FBiz().updateBjsName(uid, bjsname);
				    }
				    if(new FBiz().getBookType(uid)!=bookType){
				    	int flag=new FBiz().updateBookType(uid, bookType);
				    }
					updateTable();
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
		...
		try {
			...
			label_1.setText("共"+new FBiz().getBjsListNumber()+"项");
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
        ...
        try {
			bjsNameList=new FBiz().getBjsNameList();
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			bookTypeList=new FBiz().getBookTypeNameList();
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
